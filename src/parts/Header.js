...
export default function Header(props) {
  ...
  if (props.isCheckout) {
    return (
      <header>
        <div className="px-20 py-3 text-base flex border-b justify-center items-center">
          <div className="w-12 h-12">
            <IconNav />
          </div>
          <h3 className="ml-4 text-2xl uppercase font-medium">Warpindo</h3>
        </div>
        <div className="border-b py-3">
          <ul className="flex justify-center uppercase text-gray-800">
            <li className={`nav-item${getNavLinkClass("/")} px-8`}>
              <Button className="nav-link active" type="link" href="">
                ACCOUNT LOGIN
              </Button>
            </li>
            <li className={`nav-item${getNavLinkClass("/coffee")} px-8`}>
              <Button className="nav-link" type="link" href="/coffee">
                CART
              </Button>
            </li>
            <li className={`nav-item${getNavLinkClass("/drinks")} px-8`}>
              <Button className="nav-link" type="link" href="/drinks">
                SHIPMENT
              </Button>
            </li>
            <li className={`nav-item${getNavLinkClass("/bites")} px-8`}>
              <Button className="nav-link" type="link" href="/bites">
                METHOD PAYMENT
              </Button>
            </li>
            <li className={`nav-item${getNavLinkClass("/vinyl")} px-8`}>
              <Button className="nav-link" type="link" href="/vinyl">
                VERIFICATION
              </Button>
            </li>
            <li className={`nav-item${getNavLinkClass("/blog")} px-8`}>
